/**
 * Spotify Mapper — Converte UnifiedEvent → Spotify Ads
 *
 * Pixel: spdt('evento', { params })  →  lowercase sem separador
 * CAPI:  POST /capi-direct/events/   →  UPPER_SNAKE_CASE
 */
...
import type { SpotifyPixelLineItem } from '../spotify/pixel'
import type {
  PlatformMapper,
  PlatformPixelOutput,
  EventNameMap,
...
// Spotify Pixel: lowercase sem separador
...
// Spotify CAPI: UPPER_SNAKE_CASE
const CAPI_EVENT_NAMES: Record<UnifiedEventName, SpotifyCapiEventName> = {
  page_view: 'PRODUCT', // Spotify não tem page_view na CAPI, mapeia para PRODUCT como fallback
  view_product: 'PRODUCT',
...
function mapUserDataToSpotify(user: UnifiedUserData, hashFn: (v: string) => string): SpotifyCapiUserData {
  ...
  if (user.phone) {
    userData.hashed_phone_number = hashFn(user.phone.trim())
  }
...
function buildPixelParams(event: UnifiedEvent): SpotifyPixelParams {
  ...
  // Spotify Pixel: event_id em todos os eventos para dedup
  params.event_id = event.context.eventId

  switch (event.event) {
    case 'page_view':
      // spdt('view') — automático no base code, sem parâmetros
      break
...
function buildCapiEvent(event: UnifiedEvent, hashFn: (v: string) => string): SpotifyCapiEvent {
  ...
  // Preencher event_details baseado no tipo de evento
  switch (event.event) {
    case 'view_product':
